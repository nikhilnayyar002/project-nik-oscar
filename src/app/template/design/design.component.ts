...
export class DesignComponent {
  ...
  ngOnInit() {
    ...
    let param=this.route.snapshot.paramMap.get('id').split('-');
    this.type=param[0];
    ...
    this.db.collection('templateTypes', ref=>ref.where('id','==', param[0]))
      ...
      if(data.length) {
        ...
        if(param.length>1) {  //if 2nd array item is there
          ...
          this.create='create';
          this.templateID=(new Date()).getTime().toString().slice(-4)+this.auth.user.id.slice(0,4);   
        }
  ...
  delete() {
    ...
    function deleteFurther() {
          ...
          this.db.collection("created_templates").doc(this.templateID).delete()
          .then(()=> {
